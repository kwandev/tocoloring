...
> PRD는 페이즈별 상위 개요만 담는다. 체크리스트와 세부 작업은 `docs/roadmap/phase{1,2,3}.md` 참고.

### Phase 1 — MVP (현재)

- 홈/랜딩 페이지 (서비스 소개, 사용 방법, Before/After 예시, CTA)
- 이미지 업로드 UI (최대 2MB, jpg/png/webp)
- AI 모델을 통한 도안 변환 (결과 포맷: webp)
- Before/After 비교 슬라이더 (최신 결과 1개 표시)
- 같은 원본 재변환 (최대 2회) — 슬라이더의 After만 교체
- 변환 결과 webp 다운로드
...
- 크레딧 잔액 관리
- 크레딧 충전/사용 이력 조회
- 비로그인 사용자 IP 기반 무료 체험 (2회)

### Phase 3 — 결제 모듈

- 토스페이먼츠 연동
- 크레딧 충전 (패키지 구성, 가격은 `business.md` 참고)
- 결제 내역 관리
